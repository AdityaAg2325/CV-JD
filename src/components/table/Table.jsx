              ...
              <Table size="small" aria-label="details">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell sx={{ color: '#243c76', fontWeight: 'bold' }}>Detailed Report</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  <TableRow>
...
function processResponse(response) {
  ...
  for (const key in response.Basic_Info) {
    if (response.Basic_Info.hasOwnProperty(key) && response.Details.hasOwnProperty(key)) {
      rows.push(createData(key, response.Basic_Info[key], response.Details[key]));
    }
  }
  return rows;
}
